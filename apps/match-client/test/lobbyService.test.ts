...
// @ts-expect-error plain-JS browser module, typed by its JSDoc only
import { createLobbyService } from '../src/lobbyService.js';

const tmp = () => mkdtempSync(join(tmpdir(), 'fobal-lsvc-'));

async function until<T>(fn: () => T | Promise<T>, what: string, timeoutMs = 4000): Promise<T> {
...
async function rig(lobbyOverrides: Record<string, unknown> = {}): Promise<Rig> {
  ...
  const storeRoot = tmp();
  const secret = 'lsvc-secret';
  const options = {
    port: 0, devAuth: true, authRequestIntervalMs: 0, secret, storeRoot,
...
async function emailLogin(s: ReturnType<typeof createLobbyService>, base: string, email: string){
...
describe('LobbyService — two clients', () => {
  test('A sees B, B sees A — with the normalized identity shape', async () => {
    ...
    try {
      ...
      await emailLogin(A, r.base, 'a@fobal.ai');
      await emailLogin(B, r.base, 'b@fobal.ai');
