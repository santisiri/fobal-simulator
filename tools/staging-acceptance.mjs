...
await check('POST /matches without create key returns 401', async () => {
  const res = await fetch(`${server}/matches`, { method: 'POST', body: JSON.stringify(manifest) });
  assert(res.status === 401, `status ${res.status}`);
});
await check('POST /matches with invalid JSON returns 400', async () => {
  const res = await fetch(`${server}/matches`, { method: 'POST', headers: auth(key), body: '{nope' });
...
await check('GET result without token returns 401', async () => {
  const res = await fetch(`${server}/matches/${matchId}/result`);
  assert(res.status === 401, `status ${res.status}`);
});
await check('GET result before full time returns 404', async () => {
  const res = await fetch(`${server}/matches/${matchId}/result`, { headers: auth(tokens.spectatorToken) });
...
if (!args.fast){
  ...
  await check('repeated result reads are byte-identical (idempotent signing)', async () => {
    const [a, b] = await Promise.all([
      fetch(`${server}/matches/${matchId}/result`, { headers: auth(tokens.spectatorToken) }).then(r => r.text()),
      fetch(`${server}/matches/${matchId}/result`, { headers: auth(tokens.spectatorToken) }).then(r => r.text()),
    ]);
  ...
  await check('GET /replay returns a valid fobal-replay document', async () => {
    const res = await fetch(`${server}/matches/${matchId}/replay`, { headers: auth(tokens.spectatorToken) });
    assert(res.status === 200, `status ${res.status}`);
  ...
  await check('goal clips are served and deterministic across reads', async () => {
    const get = () => fetch(`${server}/matches/${matchId}/replays/goals`, { headers: auth(tokens.spectatorToken) });
